...
export const MODERATION_MESSAGES = {
  ...
  singleMessageLimitNotice: (user: User) =>
    `${user} Tu ne peux envoyer qu'un seul message dans ce salon. Supprime ton message precedent pour en envoyer un nouveau.`,
  sanctionDmTitle: "Moderation",
  sanctionDmDescription: (reason: string) =>
  ...
  ticketWelcomeFooter: (target: User) => `Signalement a l'encontre de ${target.username} (${target.id})`,
  ticketButtons: {
  ...
  summaryTitle: (allowModify: boolean) => (allowModify ? "Synthese IA du dossier" : "Synthese IA — version finale"),
  ...
  interactionReplies: {
    ...
    automaticallyProcessed: "Signalement traite automatiquement.",
    ...
    singleModifyOnly: "Une seule modification est autorisee pour ce ticket.",
    ...
    appealRecorded: "Appel enregistré.",
  ...
  channelPosts: {
    ...
    reportConfirmedNoSanction: "Le dossier a ete confirme sans sanction: aucune infraction n'a ete etablie. Ce salon sera supprime dans 30 secondes.",
    reportConfirmedWithSanction: "Signalement confirme et sanction appliquee. Ce salon sera supprime dans 30 secondes.",
